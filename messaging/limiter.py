...
def _swallow_future_exception(task: asyncio.Task) -> None:
    """Consume the exception of a completed fire-and-forget task.

    Without this callback, Python emits ``Future exception was never
    retrieved`` warnings when a ``fire_and_forget`` task fails.
    """
    if task.cancelled():
        return
    exc = task.exception()
    if exc is not None:
        logger.debug("fire_and_forget task failed: exc_type={}", type(exc).__name__)


def _swallow_inner_future_exception(fut: asyncio.Future) -> None:
    """Consume the exception of the inner future in ``fire_and_forget``.

    The inner future is never awaited by the caller (fire-and-forget), so
    without this callback Python would emit ``Future exception was never
    retrieved`` when ``future.set_exception`` is called on terminal failure.
    """
    if fut.cancelled():
        return
    exc = fut.exception()
    if exc is not None:
        logger.debug(
            "fire_and_forget inner future failed: exc_type={}", type(exc).__name__
        )
...
class MessagingRateLimiter:
    ...
    def fire_and_forget(
        ...
        future = asyncio.get_event_loop().create_future()
        future.add_done_callback(_swallow_inner_future_exception)
        ...
        task.add_done_callback(_swallow_future_exception)
