...
This document is a maintainer-oriented map of Claude Unbound. It explains the
...
Claude Unbound is a local proxy for agent clients. It accepts Anthropic
...
Claude Unbound optimizes for installed user workflows, not internal compatibility. The
...
[scripts/install.sh](scripts/install.sh) installs or updates the uv tool plus optional
voice extras. [scripts/uninstall.sh](scripts/uninstall.sh) removes only the uv tool and always
deletes the managed `~/.fcc/` tree from [config/paths.py](config/paths.py); they do not remove
uv, Claude Code, Codex, or uv-managed Python runtimes. [scripts/ci.sh](scripts/ci.sh) mirrors
[.github/workflows/tests.yml](.github/workflows/tests.yml) for local pre-push verification.
...
`/model`; Claude Unbound does not implement a proxy-level `/models` alias.
...
- streaming-only `/v1/responses` support for Codex/Claude Unbound workflows;
...
Claude Unbound accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed Claude Unbound/Codex workflows only need
...
Claude Unbound does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and Claude Unbound
...
- After proxy health succeeds, it fetches `/v1/models`, writes a generated Codex
  ...
  native `/model` picker lists provider slugs. Catalog generation is
