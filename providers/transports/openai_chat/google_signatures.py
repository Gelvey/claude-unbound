"""Thought-signature record/replay for Google OpenAI-compatible endpoints.

Google's Gemini (AI Studio) and Vertex AI return a ``thought_signature`` on
assistant tool-call ``extra_content`` when thinking is enabled. Subsequent
turns must replay that signature on the matching tool-call messages or the
model can reject the request. Gemini 3.x additionally requires a signature on
current-turn assistant tool calls; when the caller has none, the
``SKIP_THOUGHT_SIGNATURE_VALIDATOR`` sentinel satisfies the constraint.

This module centralizes the record/replay logic so both the Gemini and
Vertex AI providers share one implementation instead of one importing from
the other.
"""
...
MAX_TOOL_CALL_EXTRA_CONTENT_CACHE = 4096
...
def record_tool_call_extra_content(
    ...
    """Record streamed tool-call ``extra_content`` with bounded LRU eviction."""
    if tool_call_id not in cache and len(cache) >= MAX_TOOL_CALL_EXTRA_CONTENT_CACHE:
        cache.pop(next(iter(cache)))
    cache[tool_call_id] = deepcopy(extra_content)


def apply_tool_call_signatures(
    body: dict[str, Any],
    *,
    tool_call_extra_content_by_id: dict[str, dict[str, Any]] | None = None,
) -> None:
    """Replay cached signatures and fill Gemini-3 current-turn gaps on ``body``."""
...
def _apply_gemini_3_missing_current_turn_signatures(
    ...
    for message in messages[start_index + 1 :]:
        ...
        _set_tool_call_thought_signature(
            first_tool_call, SKIP_THOUGHT_SIGNATURE_VALIDATOR
        )
