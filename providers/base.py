...
class ProviderConfig(BaseModel):
    ...
    # Send a stable ``prompt_cache_key`` on OpenAI-chat requests (operator opt-in).
    include_prompt_cache_key: bool = False
    # Negotiate HTTP/2 for provider HTTP clients (operator opt-in).
    http2: bool = False


def provider_http_timeout(config: ProviderConfig) -> httpx.Timeout:
    """Build the httpx timeout shared by provider transports."""
...
def provider_http_limits(config: ProviderConfig) -> httpx.Limits:
    """Build connection-pool limits with a long keep-alive expiry.

    httpx defaults to ``keepalive_expiry=5`` seconds, so any model that takes
    longer than that per request pays a fresh TCP+TLS handshake every time.
    """
    pool_size = max(HTTP_MIN_POOL_SIZE, config.max_concurrency * 4)
    return httpx.Limits(
        max_connections=pool_size,
        max_keepalive_connections=pool_size,
        keepalive_expiry=HTTP_KEEPALIVE_EXPIRY_DEFAULT,
    )
