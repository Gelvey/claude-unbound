...
def _create_freebuff(config: ProviderConfig, _settings: Settings) -> BaseProvider:
    from providers.freebuff import FreebuffProvider

    return FreebuffProvider(config)
...
def build_provider_config(
    ...
    return ProviderConfig(
        ...
        include_prompt_cache_key=(
            descriptor.provider_id in settings.prompt_cache_key_provider_set()
        ),
        http2=settings.provider_http2,
    )
...
class ProviderRegistry:
    ...
    async def evict(self, provider_id: str) -> None:
        """Remove a cached provider and its model cache so the next ``get`` creates a fresh instance.

        Calls ``cleanup()`` on the evicted provider to release HTTP connection
        pools and other resources before discarding the reference.
        """
        provider = self._providers.pop(provider_id, None)
        self._model_ids_by_provider.pop(provider_id, None)
        ...
        if provider is not None:
            with suppress(Exception):
                try:
                    ...
                    logger.debug(
                        "Provider cleanup during evict failed: provider={} exc_type={}",
                        provider_id,
                        type(exc).__name__,
                    )
