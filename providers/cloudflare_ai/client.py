...
class CloudflareAiProvider(OpenAIChatTransport):
    ...
    def _get_retry_request_body(self, error: Exception, body: dict) -> dict | None:
        ...
        Cloudflare enforces ``input + max_tokens <= context_window`` and 400s
        instead of truncating, which breaks clients that send a large fixed
        ``max_tokens`` (e.g. Claude Code subagents on 32k-context models).
        """
        retry_body = context_length_clamped_retry_body(error, body)
        if retry_body is not None:
            logger.warning(
                "CLOUDFLARE_AI_STREAM: clamping max_tokens {} -> {} after "
    ...
    async def list_model_ids(self) -> frozenset[str]:
        ...
        # Reuse the transport's pooled client: the startup model-list refresh
        # then warms the same TCP+TLS connections later chat requests use.
