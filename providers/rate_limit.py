...
class GlobalRateLimiter:
    ...
    Proactive limits - throttles requests to stay within API limits.
      A ``rate_limit`` of 0 disables the proactive window entirely.
    ...
    def __init__(
        ...
        if rate_limit < 0:
            raise ValueError("rate_limit must be >= 0 (0 disables proactive limiting)")
        if rate_window <= 0:
        ...
        self._proactive_limiter: StrictSlidingWindowLimiter | None = (
            StrictSlidingWindowLimiter(self._rate_limit, self._rate_window)
            if rate_limit > 0
            else None
        )
        ...
        proactive = (
            f"{rate_limit} req / {rate_window}s" if rate_limit > 0 else "disabled"
        )
        logger.info(
            f"GlobalRateLimiter (Provider) initialized (proactive={proactive}, max_concurrency={max_concurrency})"
        )
    ...
    def get_instance(
        ...
        Args:
            rate_limit: Requests per window; 0 disables the proactive window
        ...
        if cls._instance is None:
            cls._instance = cls(
                rate_limit=40 if rate_limit is None else rate_limit,
    ...
    def get_scoped_instance(
        ...
        *,
        ...
        A ``rate_limit`` of 0 disables the proactive window for the scope.
        """
        if not scope:
            raise ValueError("scope must be non-empty")
        desired_rate_limit = 40 if rate_limit is None else rate_limit
    ...
    async def _acquire_proactive_slot(self) -> None:
        ...
        `self._rate_window` (seconds). No-op when the proactive window is disabled.
        """
        if self._proactive_limiter is not None:
