"""Admin managed-env persistence: read, write, validate .env files."""
...
def _load_value_state() -> dict[str, dict[str, Any]]:
    ...
    # Override Claude permission bypass from settings.json
    env_key = "CLAUDE_DANGEROUSLY_SKIP_PERMISSIONS"
    if env_key in FIELD_BY_KEY:
        from api.admin_status import read_claude_permissions_setting

        values[env_key] = "true" if read_claude_permissions_setting() else "false"
        sources[env_key] = "settings_json"
...
def load_config_response() -> dict[str, Any]:
    ...
    return {
        ...
        "provider_status": _provider_config_status(state),
    }
...
def render_env_file(values: Mapping[str, str], *, mask_secrets: bool = False) -> str:
    ...
    lines: list[str] = [
        "# Managed by Claude Unbound /admin.",
    ...
    for section in SECTIONS:
        ...
        for field in fields_by_section.get(section.section_id, []):
            if field.key == "CLAUDE_DANGEROUSLY_SKIP_PERMISSIONS":
                continue
...
def _provider_config_status(
    ...
    """Lazy proxy to :func:`api.admin_status.provider_config_status`.

    Avoids a circular module-level import (admin_status imports _load_value_state
    from this module at load time).
    """
    from api.admin_status import provider_config_status

    return provider_config_status(state)
