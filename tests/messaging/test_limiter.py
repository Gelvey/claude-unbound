...
from messaging.limiter import (
    MessagingRateLimiter,
    _swallow_future_exception,
    _swallow_inner_future_exception,
)
...
# ---------------------------------------------------------------------------
# _swallow_future_exception / _swallow_inner_future_exception tests
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_swallow_future_exception_no_exception(caplog):
    """_swallow_future_exception does nothing when the task succeeded."""
    ...
    await task
    # Should not raise and not log anything
    _swallow_future_exception(task)
    assert task.exception() is None


@pytest.mark.asyncio
async def test_swallow_future_exception_consumes_exception(caplog):
    """_swallow_future_exception retrieves and logs the task's exception."""
    ...
    with contextlib.suppress(ValueError):
        await task
    # After swallow, calling task.exception() again should work without warning
    _swallow_future_exception(task)
    assert isinstance(task.exception(), ValueError)


@pytest.mark.asyncio
async def test_swallow_future_exception_cancelled_task():
    """_swallow_future_exception handles cancelled tasks gracefully."""
    ...
    with contextlib.suppress(asyncio.CancelledError):
        await task
    # Should not raise on cancelled task
    _swallow_future_exception(task)


def test_swallow_inner_future_exception_no_exception():
    """_swallow_inner_future_exception does nothing when the future succeeded."""
    loop = asyncio.new_event_loop()
    fut = loop.create_future()
    fut.set_result("ok")
    _swallow_inner_future_exception(fut)
    assert fut.exception() is None


def test_swallow_inner_future_exception_consumes_exception(caplog):
    """_swallow_inner_future_exception retrieves and logs the future's exception."""
    loop = asyncio.new_event_loop()
    fut = loop.create_future()
...
def test_swallow_inner_future_exception_cancelled_future():
    """_swallow_inner_future_exception handles cancelled futures gracefully."""
    loop = asyncio.new_event_loop()
    fut = loop.create_future()
    fut.cancel()
    _swallow_inner_future_exception(fut)
...
@pytest.mark.asyncio
async def test_fire_and_forget_no_unretrieved_future_warning(recwarn):
    """fire_and_forget with a failing task should not emit
    'Future exception was never retrieved' warnings thanks to the
    _swallow_inner_future_exception callback."""
    await MessagingRateLimiter.shutdown_instance(timeout=0.1)
    limiter = await MessagingRateLimiter.get_instance(rate_limit=1, rate_window=1.0)
    ...
    limiter.fire_and_forget(fail_task, dedup_key="no_warn")
    await asyncio.sleep(1.5)

    # No warnings about unretrieved exceptions
    warning_messages = [str(w.message) for w in recwarn.list]
    assert not any(
        "exception was never retrieved" in msg.lower() for msg in warning_messages
    )
