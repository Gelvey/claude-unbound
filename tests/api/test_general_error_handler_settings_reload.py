...
def test_general_error_handler_emits_clean_500_when_settings_reload_raises(
    ...
    # ``settings.log_level``, and ``settings.trace_full_payloads`` during its
    # log-config call. ``lifespan_enabled=False`` keeps the app from
    # registering startup callbacks, so ``TestClient.__enter__`` cannot
    # trigger any post-create ``get_settings()`` call (which would clobber
    # the phase-2 raise-patch).
    create_app_settings = SimpleNamespace(
