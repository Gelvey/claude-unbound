...
def _pipeline_with_fake_stream(settings: Settings) -> ApiRequestPipeline:
    mock_provider = MagicMock()
    ...
    mock_provider.stream_response = fake_stream
    return ApiRequestPipeline(settings, provider_getter=lambda _: mock_provider)


def test_provider_stream_skips_raw_payload_callable_by_default():
    """The full-history dump must stay lazy: never materialized unless opted in."""
    settings = Settings()
    assert settings.log_raw_api_payloads is False
    service = _pipeline_with_fake_stream(settings)
    ...
    service._provider_stream(
        ...
        raw_log_payload=payload,
    )
    assert calls == []


def test_provider_stream_invokes_raw_payload_callable_when_opt_in():
    settings = Settings()
    settings.log_raw_api_payloads = True
    service = _pipeline_with_fake_stream(settings)
    routed = service._model_router.resolve_messages_request(
