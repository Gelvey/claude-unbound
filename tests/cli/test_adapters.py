...
def test_claude_adapter_builds_new_task_command_and_env() -> None:
    ...
    assert invocation.env["MAX_THINKING_TOKENS"] == "16000"
...
def test_claude_adapter_launcher_env_targets_proxy() -> None:
    ...
    assert env["MAX_THINKING_TOKENS"] == "16000"
    assert "ANTHROPIC_API_KEY" not in env


def test_claude_adapter_preserves_user_max_thinking_tokens() -> None:
    env = CLAUDE_CLI_ADAPTER.build_launcher_env(
        proxy_root_url="http://127.0.0.1:9191",
        auth_token="tok",
        base_env={"MAX_THINKING_TOKENS": "4096"},
    )
    ...
    assert invocation.env["MAX_THINKING_TOKENS"] == "4096"
...
def test_claude_adapter_omits_skip_permissions_by_default() -> None:
    invocation = CLAUDE_CLI_ADAPTER.build_task_invocation(
        config=_config(),
        request=CliTaskRequest(prompt="hello"),
        base_env={},
...
def test_claude_adapter_omits_skip_permissions_on_resume() -> None:
    invocation = CLAUDE_CLI_ADAPTER.build_task_invocation(
        config=_config(),
        request=CliTaskRequest(
            prompt="continue",
...
def test_codex_adapter_omits_bypass_approvals_when_disabled() -> None:
    invocation = CODEX_CLI_ADAPTER.build_task_invocation(
        config=_config(
