...
@pytest.mark.asyncio
async def test_provider_registry_evict_removes_provider_and_caches() -> None:
    """evict() removes the provider instance and all its model caches."""
    reg = ProviderRegistry()
    provider = MagicMock()
    provider.cleanup = AsyncMock()
    reg._providers["nvidia_nim"] = provider
    reg._model_ids_by_provider["nvidia_nim"] = frozenset({"model-a"})
    reg._model_infos_by_provider["nvidia_nim"] = {"model-a": MagicMock()}

    await reg.evict("nvidia_nim")

    assert "nvidia_nim" not in reg._providers
    assert "nvidia_nim" not in reg._model_ids_by_provider
    assert "nvidia_nim" not in reg._model_infos_by_provider
    provider.cleanup.assert_awaited_once()


@pytest.mark.asyncio
async def test_provider_registry_evict_calls_cleanup_on_provider() -> None:
    """evict() calls cleanup() on the evicted provider to release resources."""
    reg = ProviderRegistry()
    provider = MagicMock()
    provider.cleanup = AsyncMock()
    reg._providers["deepseek"] = provider
    reg._model_ids_by_provider["deepseek"] = frozenset({"ds-model"})
    reg._model_infos_by_provider["deepseek"] = {"ds-model": MagicMock()}

    await reg.evict("deepseek")

    provider.cleanup.assert_awaited_once()
    assert reg.is_cached("deepseek") is False
    assert reg.cached_model_ids().get("deepseek") is None


@pytest.mark.asyncio
async def test_provider_registry_evict_nonexistent_is_noop() -> None:
    """evict() on a provider_id that was never cached is a safe no-op."""
    reg = ProviderRegistry()

    await reg.evict("nonexistent")

    assert reg._providers == {}


@pytest.mark.asyncio
async def test_provider_registry_evict_swallows_cleanup_exception() -> None:
    """evict() does not raise if the provider's cleanup() fails."""
    reg = ProviderRegistry()
    provider = MagicMock()
    provider.cleanup = AsyncMock(side_effect=RuntimeError("cleanup failed"))
    reg._providers["groq"] = provider

    await reg.evict("groq")

    assert "groq" not in reg._providers
    provider.cleanup.assert_awaited_once()
