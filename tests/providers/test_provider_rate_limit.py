...
class TestProviderRateLimiter:
    ...
    @pytest.mark.asyncio
    async def test_init_rate_limit_negative_raises(self):
        """rate_limit < 0 raises ValueError."""
        GlobalRateLimiter.reset_instance()
        with pytest.raises(ValueError, match="rate_limit must be >= 0"):
            GlobalRateLimiter(rate_limit=-1, rate_window=60)

    @pytest.mark.asyncio
    async def test_rate_limit_zero_disables_proactive_window(self):
        """rate_limit=0 disables the proactive window: no throttling at all."""
        GlobalRateLimiter.reset_instance()
        limiter = GlobalRateLimiter.get_instance(rate_limit=0, rate_window=0.25)
        ...
        assert duration < 0.5, f"Disabled limiter still throttled: {duration:.2f}s"

    @pytest.mark.asyncio
    async def test_rate_limit_zero_keeps_reactive_blocking(self):
        """rate_limit=0 keeps reactive 429/5xx backoff active."""
        GlobalRateLimiter.reset_instance()
        limiter = GlobalRateLimiter.get_instance(rate_limit=0, rate_window=60)

        limiter.set_blocked(0.2)
    ...
    @pytest.mark.asyncio
    async def test_scoped_instance_rate_limit_zero_not_coerced_to_default(self):
        """get_scoped_instance must treat 0 as disabled, not fall back to 40."""
        GlobalRateLimiter.reset_instance()
        limiter = GlobalRateLimiter.get_scoped_instance(
            "zero_scope", rate_limit=0, rate_window=60
        )
        assert limiter._rate_limit == 0
        assert limiter._proactive_limiter is None
        assert limiter.matches_config(0, 60, 5)
        # Same config returns the same instance (no rebuild churn)
        assert (
