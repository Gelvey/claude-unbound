...
class TestSSEBuilderMessageLifecycle:
    ...
    def test_message_delta_provider_input_and_cache_read_override(self):
        builder = SSEBuilder("msg_1", "model", input_tokens=999)
        sse = builder.message_delta(
            "end_turn", 42, input_tokens=100, cache_read_input_tokens=700
        )
        data = _parse_sse(sse)
        assert data["usage"]["input_tokens"] == 100
        assert data["usage"]["cache_read_input_tokens"] == 700
        assert data["usage"]["output_tokens"] == 42

    def test_message_delta_omits_cache_read_when_absent_or_zero(self):
        builder = SSEBuilder("msg_1", "model", input_tokens=5)
        for kwargs in ({}, {"cache_read_input_tokens": 0}):
            data = _parse_sse(builder.message_delta("end_turn", 1, **kwargs))
...
class TestSSEBuilderTokenEstimation:
    ...
    def test_estimate_without_tiktoken(self):
        ...
        with patch("core.anthropic.sse._encoder_or_none", return_value=None):
            tokens = builder.estimate_output_tokens()
