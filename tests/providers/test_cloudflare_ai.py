...
async def test_list_model_ids_fetches_from_api(cloudflare_provider):
    ...
    with patch.object(cloudflare_provider, "_http_client") as mock_client:
        mock_client.get = AsyncMock(return_value=mock_response)

        model_ids = await cloudflare_provider.list_model_ids()

    # The shared (warm) transport client is reused; no ephemeral client per call.
    mock_client.get.assert_awaited_once()
    assert mock_client.get.call_args.kwargs["headers"] == {
        "Authorization": "Bearer test_cloudflare_token"
    }
