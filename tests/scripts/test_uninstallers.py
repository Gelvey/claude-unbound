...
def test_readme_uninstall_one_liners_use_raw_github_urls() -> None:
    ...
    assert (
        ...
        'Gelvey/claude-unbound/main/scripts/uninstall.sh" | sh'
    ) in text
    assert (
        'irm "https://raw.githubusercontent.com/'
        'Gelvey/claude-unbound/main/scripts/uninstall.ps1" | iex'
    ) in text
...
def _write_mock_no_procs(bin_dir: Path) -> None:
    """Write mock pgrep/ps that never find running processes.

    The uninstaller checks for running fcc-server/fcc-claude processes via
    pgrep or ps. In a real dev environment those processes may actually be
    running, causing the guard to abort the script. These mocks ensure the
    tests exercise the uninstall logic, not the process guard.
    """
    for name, body in (
        ("pgrep", "#!/bin/sh\nexit 1\n"),
        ("ps", "#!/bin/sh\nexit 0\n"),
...
def test_uninstall_sh_fails_when_fcc_commands_are_running() -> None:
    ...
    assert "Claude Unbound is still running" in guard_body
    assert (
...
def test_uninstall_sh_missing_uv_still_deletes_fcc_home(tmp_path: Path) -> None:
    ...
    fcc_home.mkdir(parents=True)
    empty_bin.mkdir()
    _write_mock_no_procs(empty_bin)
