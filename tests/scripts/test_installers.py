...
def _run_install_sh(*args: str) -> subprocess.CompletedProcess[str]:
    ...
    # when CI is executing from a non-upstream fork clone (e.g. Gelvey/claude-unbound).
    env = os.environ.copy()
    env.setdefault(
        "FCC_REPO_URL",
        "git+https://github.com/Gelvey/claude-unbound.git",
    )
...
def test_install_sh_dry_run_produces_expected_steps() -> None:
    ...
    assert "Cloning or updating Claude Unbound repository" in stdout
