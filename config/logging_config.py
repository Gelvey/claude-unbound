...
def _stdlib_level(level: str) -> int:
    """Map a loguru level name to the closest stdlib logging level."""
    if level == "TRACE":
...
def configure_logging(
    ...
    *,
    ...
    ``level`` sets the minimum level for both the JSON sink and stdlib root logger
    ...
    sink_level = str(level).strip().upper() or "INFO"
    ...
    logging.root.setLevel(_stdlib_level(sink_level))
