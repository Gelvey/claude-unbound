...
- Before pushing, run `./scripts/ci.sh` to run all GitHub CI checks locally; requires `uv` on PATH.
- Use `--only` / `--skip` to run a subset when iterating; use `--dry-run` to print commands without running them.
...
4. **VERIFY**: Run `./scripts/ci.sh`, plus relevant smoke tests when needed. Confirm the fix via logs or output.
...
- `scripts/install.sh`, `scripts/uninstall.sh`, `scripts/ci.sh`
