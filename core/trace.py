...
By default, long text values inside body snapshots are truncated to short
previews; call :func:`configure_trace_payloads` with ``True`` (settings
``TRACE_FULL_PAYLOADS``) to log conversation text verbatim. Values under
sanitized credential keys (e.g. ``api_key``, ``authorization``) are always
redacted.
"""
...
_TRACE_TEXT_PREVIEW_CHARS = 200

_full_payload_tracing = False


def configure_trace_payloads(full_payloads: bool) -> None:
    """Set whether body snapshots carry verbatim text or truncated previews."""
    global _full_payload_tracing
    _full_payload_tracing = bool(full_payloads)


def _truncate_long_strings(obj: Any) -> Any:
    """Recursively replace long strings with short previews plus lengths."""
    if isinstance(obj, str):
        if len(obj) <= _TRACE_TEXT_PREVIEW_CHARS:
            return obj
        return f"{obj[:_TRACE_TEXT_PREVIEW_CHARS]}... [{len(obj)} chars]"
    if isinstance(obj, Mapping):
        return {str(k): _truncate_long_strings(v) for k, v in obj.items()}
    if isinstance(obj, tuple | list):
        return [_truncate_long_strings(x) for x in obj]
    return obj


def _snapshot_trace_value(obj: Any) -> Any:
    """Sanitize a body snapshot; truncate long text unless full tracing is on."""
    sanitized = _sanitize_trace_value(obj)
    if _full_payload_tracing:
        return sanitized
    return _truncate_long_strings(sanitized)
...
def api_messages_request_snapshot(req: Any) -> dict[str, Any]:
    ...
    return _snapshot_trace_value(snapshot)
...
def provider_chat_body_snapshot(body: Mapping[str, Any]) -> dict[str, Any]:
    """Sanitized OpenAI-compat chat body subset for traces (text truncated by default)."""
    ...
    return _snapshot_trace_value(snap)
